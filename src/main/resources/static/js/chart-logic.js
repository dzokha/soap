...
/**
 * SOAP - D3.js Chart Logic
 * File này chứa các hàm vẽ biểu đồ sinh tin học mô phỏng phần mềm FastQC.
 * Yêu cầu: Đã nhúng thư viện D3.v7 (https://d3js.org/d3.v7.min.js)
 */

const CHART_CONFIG = {
    // Tăng margin bottom lên 65 để chứa các nhãn chữ nghiêng 45 độ
...
// ============================================================================
// HÀM TIỆN ÍCH (Vẽ khung biểu đồ cơ bản)
// ============================================================================
function createChartSvg(selector) {
    const container = d3.select(selector);
    container.selectAll("*").remove(); // Xóa biểu đồ cũ nếu có
...
function drawGridAndAxes(g, xScale, yScale, innerWidth, innerHeight, xAxisLabel, yAxisLabel, isPointScale = false, dataLength = 0) {
    ...
    // FIX LỖI: Tự động bỏ bớt nhãn nếu dữ liệu quá dài (> 15 điểm)
    if (isPointScale && dataLength > 15) {
        const step = Math.ceil(dataLength / 15); // Chỉ giữ lại tối đa ~15 nhãn
    ...
    // Xoay nghiêng nhãn 45 độ nếu bị bỏ bớt (giúp dễ đọc)
    if (shouldRotate) {
    ...
    g.append("text")
        .attr("x", innerWidth / 2)
        .attr("y", innerHeight + (shouldRotate ? 55 : 40)) // Dời xuống nếu nhãn bị xoay nghiêng
        .attr("text-anchor", "middle")
...
/**
 * Hàm vẽ biểu đồ Adapter Content chuẩn FastQC
 */
function drawAdapterChart(selector, data) {
    if (!data || data.length === 0) return;
...
/**
 * Hàm vẽ biểu đồ Per Base Sequence Quality (Box-and-Whisker)
 */
function drawPerBaseQualityChart(selector, data) {
    if (!data || data.length === 0) return;
    ...
    // FIX LỖI: Trục X của Quality Chart
...
// ============================================================================
function drawPerSequenceQualityChart(selector, data) {
    if (!data || data.length === 0) return;
...
// ============================================================================
function drawPerBaseSequenceContentChart(selector, data) {
    if (!data || data.length === 0) return;
    const { g, innerWidth, innerHeight } = createChartSvg(selector);
    ...
    // Bật cờ isPointScale = true và truyền vào độ dài dữ liệu để hàm tự tính toán giảm nhãn
...
// ============================================================================
function drawPerSequenceGCContentChart(selector, data) {
    if (!data || data.length === 0) return;
    const { g, innerWidth, innerHeight } = createChartSvg(selector);
...
// ============================================================================
function drawPerBaseNContentChart(selector, data) {
    if (!data || data.length === 0) return;
    const { g, innerWidth, innerHeight } = createChartSvg(selector);
...
// ============================================================================
function drawSequenceLengthDistributionChart(selector, data) {
    if (!data || data.length === 0) return;
    const { g, innerWidth, innerHeight } = createChartSvg(selector);
...
// ============================================================================
function drawDuplicationLevelsChart(selector, data) {
    if (!data || data.length === 0) return;
    const { g, innerWidth, innerHeight } = createChartSvg(selector);
...
// ============================================================================
// 7. Overrepresented Sequences (Table)
// ============================================================================
function drawOverrepresentedTable(selector, data) {
